...
class RobertaBBHQATucker:
    ...
    def validate(predictions, split=0.2):
        ...
        val_acc, val_logloss = RobertaBBHQATucker.get_acc_log_loss(validation_predictions)
        test_acc, test_logloss = RobertaBBHQATucker.get_acc_log_loss(test_predictions)
    ...
    def intervene(self, model, tokenizer, dataset, intervention_mode, layer, rank, decomposition_type='tucker'):
        ...
        BATCH_SIZE = 128
        self.logger.log(f"Dataset size {dataset_size}. Batch size {BATCH_SIZE}")
...
if __name__ == '__main__':
    ...
    experiment = RobertaBBHQATucker(save_dir=save_dir, logger=logger)
    ...
    for layer in layers:
        for rank in ranks:
            ...
            results_df.to_csv(f"/home/hpate061/Laser_T/results/TASER_ROBERTA:{intervention_mode}_BBHQA_Tucker_layer11.csv", index=False)
            print(f"Layer: {layer}, Rank: {rank}, Results: {results.to_str()}")
